...
export abstract class NamedLogger implements ILogger, ILoggerSwitchable {
  ...
  enable(overrideName: string | null = null) {
    ...
    this.log = this._name ? (...args) => this.implementation.log(this._name, ...args) : this.implementation.log;

    this.warn = this._name ? (...args) => this.implementation.warn(this._name, ...args) : this.implementation.warn;

    this.error = this._name ? (...args) => this.implementation.error(this._name, ...args) : this.implementation.error;
